...
* DB
  - database
* DB에서 다시 결과값 받아서 웹페이지에 출력합니다.
  - web

### 시스템 조건

- 인터넷 연결이 가능한 환경이어야 합니다.
- 백엔드 서버가 실행되고 있는 상태여야 합니다.
- 사용자가 원하는 프로그래밍 언어를 선택한 상태여야 합니다.
...
&nbsp;이러한 경우에 저희가 개발한 서비스를 사용한다면, **다양한 언어의 공부 및 개발에서 사용자에게 높은 접근성과 편의성을 제공할 수 있습니다.** 따라서, PC가 없는 상황에서 모바일 및 태플릿으로 할 수 있는 간단한 프로그래밍, 각종 프로그래밍 수업에서 교육자 간의 통일된 환경을 보장하고 싶을 때, 해당 서비스는 높은 가치를 지니게 될 것입니다.
